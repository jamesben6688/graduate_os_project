...
class Spiffs(object):
    ...
    def fopen(self, filename, flags):
       # print("free blocks", self.fs.free_blocks)
    ...
    def dir(self):
        ...
        # print(size(ctypes.c_char_p), "lalalal")
        # print(ctypes.c_uint32)
        # print(ctypes.c_uint32)
...
def check_flag(spiffs_mount, page_limit):
    global reach_page_limit
    total_size = 0
    filelist = spiffs_mount.dir()
...
def move_data():
    ...
    for file in filelist:
        total_size += file[1]
        fd = s.open(file[0])
        file_to_move.append([file[0], fd.read()])
        fd.close()
        s.remove(file[0])
...
def _tests(spiffs_mount):
    ...
    s.fwrite(fd, "Hello World")  # check the varaible(size), not full, full-> send signal to you. move func, remove all the system
    check_flag(spiffs_mount, 10)
    #print s.open("Hello").read()
    ...
    # print s.dir()   # list all the files, filename, filesize, fileid
...
if __name__=="__main__":
    ...
    file('workspace/back.bin','w').write('\xff'*8*1024*1024)
